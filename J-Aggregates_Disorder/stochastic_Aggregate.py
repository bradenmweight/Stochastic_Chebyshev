...
def get_Globals():
    ...
    global SIG, ESIG
    SIG  = 0.1 # Width of Gaussian energy disorder
    ESIG = np.random.normal( E0, SIG, size=N )
...
def do_STOCHASTIC_DOS():
    ...
    for pt in range( NPTS ):
        ...
        for _ in range( N_STOCHASTIC ): # WE COULD PARALLELIZE THIS EASILY
            ...
            v0 = r_vec
            v1 = get_H_vec_norm( E0,J,r_vec,dH,N,ESIG )
...
def get_vec_Tn_vec( nmax, r_vec, N, v0, v1 ):
    ...
    |v_n+1> = 2H|v_n> - |v_n-1>
    """
    v2 = 2 * get_H_vec_norm( E0,J,v1,dH,N,ESIG ) - v0 # Recurssion Relation
...
def plot_Disorder( ESIG ):
    ENERGY = np.linspace(EMIN,EMAX,NPTS)
    DOS  = np.zeros( NPTS )
    for pt in range( NPTS ):
        E       = EMIN + pt*dE
        DOS[pt] = np.sum( np.exp( -(E - ESIG[:])**2 / 2 / GAM**2 ) )
    print(DOS)
    plt.plot( ENERGY, DOS / np.max(DOS), c="red" )
    plt.xlabel("Energy (a.u.)", fontsize=15)
    plt.ylabel("Density of States (Arb. Units)", fontsize=15)
    plt.title("$N_\mathrm{sites} = %s$   $N_\mathrm{random} = %1.0f$   $N_\mathrm{Chebyshev} = %1.0f$" % (N,N_STOCHASTIC,N_CHEB), fontsize=15)
    plt.tight_layout()
    plt.savefig("%s/Disorder_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f_J_%1.3f_SIG_%1.3f.jpg" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM,J,SIG), dpi=300)
    plt.clf()
...
def plot_DOS( EXACT, STOCHASTIC ):
    ...
    plt.tight_layout()
    plt.savefig("%s/DOS_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f_J_%1.3f_SIG_%1.3f.jpg" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM,J,SIG), dpi=300)
    plt.clf()

def main():
    get_Globals()
    plot_Disorder( ESIG )
    DOS_EXACT      = do_H_EXACT( N, NPTS, E0, GAM, J, EMIN, EMAX, dE, dH, ESIG )
