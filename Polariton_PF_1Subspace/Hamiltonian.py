...
@jit(nopython=True,fastmath=True)
def get_H_nm_norm_TRANSITION( N, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0, dH, E0, n, m ):
    H = 0.0
    if ( n == m ): # DIAGONAL ELEMENTS
        if ( n == 0 ):               # <g,g,0|H|g,g,0>
            H  = EGS
...
@jit(nopython=True,fastmath=True)
def get_H_vec_norm_TRANSITION( N, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0, dH, E0, vec ):
    Hvec         = np.zeros( (N+2) )

    # TOP ROW
    Hvec[0]     += (EGS-E0)*vec[0] # <g0|g0>
    Hvec[0]     += np.sum( DSE_0n[:] * vec[1:N+1] ) # <g0|e0> # TODO CHECK THIS
    Hvec[0]     += WC * A0 * np.sum( MU[:,0,0] ) * vec[N+1] #<g0|g1> # TODO CHECK THIS
    ...
    for n in range( 1, N+1 ): # MIDDLE ROWS
        ...
        # FIRST COLUMN
        Hvec[n] += (DSE_0n[n-1] - DSE_GS) * vec[0]

        # LEFT-TO-DIAGONAL COLUMNS
        for m in range( 2, n ):
            Hvec[n] += 2 * WC * A0**2 * MU[n-1,0,1] * MU[m-1,0,1] * vec[m]
        #Hvec[n] += 2 * WC * A0**2 * MU[n-1,0,1] * (np.sum( MU[:n-1,0,1] * vec[1:n] ) ) #- MU[n-1,0,0] * vec[n] )
