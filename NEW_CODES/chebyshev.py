...
if ( __name__ == "__main__" ):
    ...
    params = Params( F_type='Gaussian', N_CHEB=N_CHEB, dH=1.0, GAM=0.01 )
    ...
    params = Params( F_type='lorentzian', N_CHEB=N_CHEB, dH=1.0, GAM=0.01 )
    coeffs = get_coeffs( params )
    coeffs = normalize_coeffs( coeffs )
    L_EFF  = np.sum(np.arange( N_CHEB ) * np.abs(coeffs)**2)
    L2_EFF  = np.sqrt(np.sum(np.arange( N_CHEB )**2 * np.abs(coeffs)**2))
    plt.plot( np.abs(coeffs)**2, "o", ms=4, c="red", label='Lorentzian ($\\langle L \\rangle$ = %1.1f  $\\sqrt{\\langle L^2 \\rangle}$ = %1.1f)' % (L_EFF, L2_EFF) )
    params = Params( F_type='1_over_E', N_CHEB=N_CHEB, dH=1.0, GAM=0.01 )
    coeffs = get_coeffs( params )
    coeffs = normalize_coeffs( coeffs )
    L_EFF  = np.sum(np.arange( N_CHEB ) * np.abs(coeffs)**2)
    L2_EFF  = np.sqrt(np.sum(np.arange( N_CHEB )**2 * np.abs(coeffs)**2))
    plt.plot( np.abs(coeffs)**2, lw=2, c="blue", label='1_over_E ($\\langle L \\rangle$ = %1.1f  $\\sqrt{\\langle L^2 \\rangle}$ = %1.1f)' % (L_EFF, L2_EFF) )
