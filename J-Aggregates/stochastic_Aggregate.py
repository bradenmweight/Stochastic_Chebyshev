...
def do_STOCHASTIC_DOS():
    ...
    plt.tight_layout()
    plt.savefig("%s/EXPANSION_COEFFS_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f.jpg" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM), dpi=300)
    plt.clf()
...
def plot_DOS( EXACT, STOCHASTIC ):
    ...
    plt.tight_layout()
    plt.savefig("%s/DOS_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f.jpg" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM), dpi=300)
    plt.clf()
