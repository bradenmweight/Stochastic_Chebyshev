...
def get_Globals():
    ...
    global N, EMOL, dEMOL_SHIFT, MU, MU_SCALED, WC, WC, A0, A0, A0_SCALED, E0, SIGE, SIGG, CAV_LOSS
    ...
    GAM    = 0.010
    N_STOCHASTIC = 500 # Num. random vectors. Conserving Error: N = 1000 and Nr = 5000 --> E. N = 10**6 and E --> Nr = 5
    N_CHEB       = 400  # Number of Chebyshev expansion coefficients
    EMIN   =  0.5 - SIGE # Must be lower than smallest eigenvalue
    EMAX   =  1.5 + SIGE # Must be higher than largest eigenvalue
    dH     = (EMAX-EMIN)
...
def do_STOCHASTIC_DOS():
    ...
    DOS_FULL = np.zeros( (NPTS,3), dtype=np.complex128 ) # Total, Matter, Photonic
    for pt in range( NPTS ): # SHOULD WE PARALLIZE THIS ? THIS IS THE MOST TIME-CONSUMING PART
        ...
        T0 = time()
        DOS_FULL[pt,:] = do_Stochastic_Chebyshev( c_l, N, dEMOL_SHIFT, pt, MU_SCALED, CAV_LOSS, WC, dH, E0 )
...
def do_STOCHASTIC_DOS_PARALLEL( pt ):
    ...
    DOS_FULL = np.zeros( (3), dtype=np.complex128 ) # Total, Matter, Photonic
    c_l      = np.zeros( N_CHEB, dtype=np.complex128 )
    c_l      = get_coeffs( EGRID[pt], E0, N_CHEB, c_l )#.real
    DOS_FULL = do_Stochastic_Chebyshev( c_l, N, dEMOL_SHIFT, pt, MU_SCALED, CAV_LOSS, WC, E0, dH )
    return DOS_FULL / 2 / np.pi

@jit(nopython=True)
def do_Stochastic_Chebyshev( c_l, N, dEMOL_SHIFT, pt, MU_SCALED, CAV_LOSS, WC, E0, dH ):
    for r in range( N_STOCHASTIC ):
        #print(r)
        #Tr = time()
        #r_vec = np.array([ random.randint(0,1)*2.-1. for n in range(N+1) ]).astype(np.complex128)
        r_vec = np.array([ np.exp(1j * 2 * np.pi* np.random.uniform(-1,1)) for n in range(N+1) ]).astype(np.complex128)
        v0, v1, DOS = get_T0_T1_vec( c_l, N, dEMOL_SHIFT, MU_SCALED, CAV_LOSS, WC, E0, dH, r_vec )
        for l in range( 2, N_CHEB ):
            v0, v1, RN = get_vec_Tn_vec( N, dEMOL_SHIFT, MU_SCALED, CAV_LOSS, WC, E0, dH, r_vec, v0, v1 )
            DOS += c_l[l] * RN[:]
        #print( "\tr %s of %s  Time: %1.2f s  Remaining Time: %1.2f hrs"  % (r, N_STOCHASTIC, time() - Tr, (time() - Tr)*( N_STOCHASTIC - r-1 )/3600) )
    
    print( EGRID[pt], DOS.real / N_STOCHASTIC )
    return DOS / N_STOCHASTIC


@jit(nopython=True,fastmath=True)
def get_T0_T1_vec( c_l, N, dEMOL_SHIFT, MU_SCALED, CAV_LOSS, WC, E0, dH, r_vec ):
    v0 = r_vec
    v1 = get_H_vec_norm_Hermitian( N, CAV_LOSS, dEMOL_SHIFT, MU_SCALED, WC, E0, dH, v0 )
    DOS     = np.zeros( (3), dtype=np.complex128 )
...
@jit(nopython=True)
def get_vec_Tn_vec( N, dEMOL_SHIFT, MU_SCALED, CAV_LOSS, WC, E0, dH, r_vec, v0, v1 ):
    """
    Returns: <r|T_n(H)|r_n>
    ...
    #T0 = time()
    TMP = get_H_vec_norm_Hermitian( N, CAV_LOSS, dEMOL_SHIFT, MU_SCALED, WC, E0, dH, v1 )
    v2  = 2 * TMP - v0 # Recurssion Relation
    #print("Time (H @ vec):", time() - T0)

    #T0 = time()
    R    = np.zeros( (3), dtype=np.complex128 )
    R[0] = np.dot(     np.conjugate(r_vec),     v2 ) # <r| * (T_n(H)|r>)
    R[1] = np.dot( np.conjugate(r_vec[Mi]), v2[Mi] ) # <M| * (T_n(H)|r>) * <r|M>
    R[2] = np.dot( np.conjugate(r_vec[Pi]), v2[Pi] ) # <P| * (T_n(H)|r>) * <r|P>
...
def plot_DOS( EXACT, STOCHASTIC ):
    ...
    ### PHOTON DOS ###
    plt.plot( EGRID.real, np.abs(STOCHASTIC[:,2]), "o", c="black", label="STOCHASTIC" )
    #plt.plot( EGRID, STOCHASTIC[:,0], "o", c="red", label="STOCHASTIC" )
    if ( EXACT[0] is not None ):
...
def main():
    get_Globals()
    DOS_EXACT_T, DOS_EXACT_M, DOS_EXACT_P = do_H_EXACT( N, NPTS, EMOL, MU, CAV_LOSS, WC, A0, EGRID, dH, E0, GAM )
