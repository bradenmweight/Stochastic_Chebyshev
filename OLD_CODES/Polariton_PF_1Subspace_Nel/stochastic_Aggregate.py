...
def get_Globals():
    ...
    global N, NEL, E, MU, WC, A0, A0_SCALED, EGS, E0, SIGE, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n
    N         = int(sys.argv[1]) # Number of molecules
    NEL       = 3 # Number of electronic states, including GS
    WC        = float(sys.argv[3])
    A0        = float(sys.argv[2])
    E         = np.zeros( (N,NEL) )
    MU        = np.zeros( (N,NEL,NEL) )
    SIGE      = 0.0
    E[:,1]    = 1.0 + np.random.normal( 0.0, SIGE, size=N )
    MU        = np.ones( (N,NEL,NEL) )
    MU[:,0,1] = 1
    MU[:,0,0] = 1/100
    MU[:,1,1] = 1/100
    MU[:,2,2] = 1/100
    # No dipole coupling to third electronic state
    ...
    P[1:N+1] = np.zeros( (N*(NEL-1)) ) # Excited matter is not photon.
...
def do_STOCHASTIC_DOS():
    ...
    for pt in range( NPTS ): # SHOULD WE PARALLIZE THIS ? THIS IS THE MOST TIME-CONSUMING PART
        ...
        T0 = time()
        DOS[pt,:] = do_Stochastic_Chebyshev( DOS[pt,:], c_l, N, EL, EGS, E, pt, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, M, P )
...
def do_STOCHASTIC_DOS_PARALLEL( pt ):
    ...
    print("Time to get coefficients: %1.3f s" % (time() - T0))
    T0 = time()
    DOS = do_Stochastic_Chebyshev( DOS, c_l, N, EL, EGS, E, pt, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, M, P )
    print("Stochastic Time: %1.3f s" % (time() - T0) )
...
@jit(nopython=True)
def do_Stochastic_Chebyshev( DOS, c_l, N, EL, EGS, E, pt, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, M, P ):
    for _ in range( N_STOCHASTIC ):
        ...
        v0, v1, DOS = get_T0_T1_vec( DOS, pt, c_l, N, EL, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, r_vec, M, P )
        for l in range( 2, N_CHEB ):
            v0, v1, RN = get_vec_Tn_vec( N, EL, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, r_vec, M, P, v0, v1 )
            DOS += c_l[l] * RN[:]
    return DOS


@jit(nopython=True,fastmath=True)
def get_T0_T1_vec( DOS, pt, c_l, N, EL, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, r_vec, M, P ):
    v0 = r_vec
    v1 = get_H_vec_norm( N, EL, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, r_vec )
...
@jit(nopython=True)
def get_vec_Tn_vec( N, EL, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, r_vec, M, P, v0, v1 ):
    """
    Returns: <r|T_n(H)|r_n>
    ...
    #T0 = time()
    TMP = get_H_vec_norm( N, EL, EGS, E, MU, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, v1 )
