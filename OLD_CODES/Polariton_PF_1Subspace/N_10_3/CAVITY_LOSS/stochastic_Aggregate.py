...
def get_Globals():
    ...
    GAM    = 0.05
    N_STOCHASTIC = 2000
    N_CHEB       = 150 # Number of Chebyshev expansion coefficients
...
def do_STOCHASTIC_DOS():
    ...
    for pt in range( NPTS ): # SHOULD WE PARALLIZE THIS ? THIS IS THE MOST TIME-CONSUMING PART
        ...
        T0 = time()
        DOS[pt,:] = do_Stochastic_Chebyshev( DOS[pt,:], c_l, N, EGS, E, pt, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, M, P )
    ...
    return DOS, DOS_NonHermitian*0.0

def do_STOCHASTIC_DOS_PARALLEL( pt ):

    DOS              = np.zeros( (3), dtype=np.complex64 ) # Total, Matter, Photonic
    DOS_NonHermitian = np.zeros( (3), dtype=np.complex64 ) # Total, Matter, Photonic
    print( "%1.0f of %1.0f" % (pt, NPTS) )
    c_l = np.zeros( N_CHEB, dtype=np.complex64 )
    T0 = time()
    c_l = get_coeffs( EGRID[pt], E0, N_CHEB, c_l )#.real # TODO IF WE ADD COMPLEX HAMILTONIAN, WE NEED TO CHANGE THIS
    print("Time to get coefficients: %1.3f s" % (time() - T0))
    T0 = time()
    DOS = do_Stochastic_Chebyshev( DOS, c_l, N, EGS, E, pt, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, M, P )
...
def do_Diagonal_Chebyshev_Non_Hermitian( DOS_NonHermitian, c_l, N, EGS, E, pt, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, M, P ):
    for an in range( N+2 ):
        v0     = np.zeros( (N+2), dtype=np.complex64 )
        v0[an] = 1.0
...
def get_T0_T1_vec( DOS, pt, c_l, N, EGS, E, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, r_vec, M, P ):
    ...
    DOS[0] += c_l[0] * 1j * np.dot(     np.conjugate(r_vec) ,     r_vec ) # <r|r>
    DOS[1] += c_l[0] * 1j * np.dot( M * np.conjugate(r_vec) , M * r_vec ) # <M|r> * <r|M>
    DOS[2] += c_l[0] * 1j * np.dot( P * np.conjugate(r_vec) , P * r_vec ) # <P|r> * <r|P>
    DOS[0] += c_l[1] * 1j * np.dot(     np.conjugate(r_vec) ,     v1 )    # <r|T_1(H)|r>
    DOS[1] += c_l[1] * 1j * np.dot( M * np.conjugate(r_vec) , M * v1 )    # <M|T_1(H)|r> * <r|M>
    DOS[2] += c_l[1] * 1j * np.dot( P * np.conjugate(r_vec) , P * v1 )    # <P|T_1(H)|r> * <r|P>
    return v0, v1, DOS
...
def get_vec_Tn_vec( N, EGS, E, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0_SCALED, dH, E0, r_vec, M, P, v0, v1 ):
    ...
    #T0 = time()
    R_T = 1j * np.dot(     np.conjugate(r_vec),     v2 )          # <r| * (T_n(H)|r>)
    R_M = 1j * np.dot( M * np.conjugate(r_vec), M * v2 ) # <M| * (T_n(H)|r>) * <r|M>
    R_P = 1j * np.dot( P * np.conjugate(r_vec), P * v2 ) # <P| * (T_n(H)|r>) * <r|P>
...
def get_coeffs( E,E0,N_CHEB,c_l ):
    ...
    dth      = theta[1] - theta[0]
    F        = np.exp( -( dH*np.cos(theta) - E)**2 / 2 / GAM**2 )
    c_l[:]   = np.einsum( "t,tl->l", F, np.exp(1j * np.outer(theta,np.arange(N_CHEB))) ) * dth # Fourier Kernel
...
def plot_DOS( EXACT, STOCHASTIC_Hermitian, DOS_STOCHASTIC_Non_Hermitian ):
    ...
    plt.plot( EGRID, STOCHASTIC_Hermitian[:,2] / np.max(np.abs(STOCHASTIC_Hermitian[:,2])), "o", c="red", label="STOCHASTIC" )
    plt.plot( EGRID, np.abs(STOCHASTIC_Hermitian[:,2]) / np.max(np.abs(STOCHASTIC_Hermitian[:,2])), "--", c="red", label="STOCHASTIC (ABS)" )
    ...
    plt.clf()
    np.savetxt("%s/DOS_PHOTON_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f_A0_%1.3f_WC_%1.3f_SIGE_%1.3f_SIGG_%1.3f_CAVLOSS_%1.3f_ABS.dat" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM,A0,WC,SIGE,SIGG,CAV_LOSS), np.c_[EGRID, np.abs(STOCHASTIC_Hermitian[:,2]) / np.max(np.abs(STOCHASTIC_Hermitian[:,2]))])
    np.savetxt("%s/DOS_PHOTON_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f_A0_%1.3f_WC_%1.3f_SIGE_%1.3f_SIGG_%1.3f_CAVLOSS_%1.3f.dat" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM,A0,WC,SIGE,SIGG,CAV_LOSS), np.c_[EGRID, STOCHASTIC_Hermitian[:,2] / np.max(np.abs(STOCHASTIC_Hermitian[:,2]))])
