...
def plot_DOS( EXACT, STOCHASTIC ):
    ...
    plt.plot( EGRID.real, np.abs(STOCHASTIC[:,0].real), "-", c="red", label="STOCHASTIC (RE)" )
    plt.plot( EGRID.real, np.abs(STOCHASTIC[:,0].imag), "-", c="blue", label="STOCHASTIC (IM)" )
    #plt.plot( EGRID, STOCHASTIC[:,0], "o", c="red", label="STOCHASTIC" )
    if ( EXACT[0] is not None ):
        plt.plot( EGRID.real, EXACT[0,:].real, "-", lw=6, alpha=0.25, c="black", label="EXACT" )
    ...
    plt.clf()
    np.savetxt("%s/DOS_TOTAL_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f_A0_%1.3f_WC_%1.3f_SIGE_%1.3f_SIGG_%1.3f_CAVLOSS_%1.3f_ABS.dat" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM,A0,WC,SIGE,SIGG,CAV_LOSS), np.c_[EGRID, np.abs(STOCHASTIC[:,0]) / np.max(np.abs(STOCHASTIC[:,0]))])
    np.savetxt("%s/DOS_TOTAL_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f_A0_%1.3f_WC_%1.3f_SIGE_%1.3f_SIGG_%1.3f_CAVLOSS_%1.3f.dat" % (DATA_DIR,N,N_STOCHASTIC,N_CHEB,GAM,A0,WC,SIGE,SIGG,CAV_LOSS), np.c_[EGRID, STOCHASTIC[:,0] / np.max(np.abs(STOCHASTIC[:,0]))])
