...
def do_H_EXACT( N, NPTS, EGS, E, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0, EMIN, EMAX, EGRID, dH, E0, GAM ):
    ...
    for pt in range( NPTS ):
        DOS_T[pt] = np.sum( np.exp( -(EGRID[pt] - Ei[:].real)**2 / 2 / GAM**2 ) )
        DOS_M[pt] = np.sum( ELEC[:] * np.exp( -(EGRID[pt] - Ei[:].real)**2 / 2 / GAM**2 ) )
        DOS_P[pt] = np.sum( PHOT[:] * np.exp( -(EGRID[pt] - Ei[:].real)**2 / 2 / GAM**2 ) )
