...
def get_H_vec_norm_Hermitian( N, EGS, E, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0, dH, E0, vec ):
    ...
    Hvec[0] = (1.0 - E0) * vec[0] + 0.1                          * vec[1] + 0.0 * vec[2]
    Hvec[1] = 0.1        * vec[0] + (WC  - E0 - CAV_LOSS/2*1.0j) * vec[1] + 0.0 * vec[2]
    #Hvec[2] = 0.0        * vec[0] + 0.0                          * vec[1] + 0.0 * vec[2]
    return Hvec/dH