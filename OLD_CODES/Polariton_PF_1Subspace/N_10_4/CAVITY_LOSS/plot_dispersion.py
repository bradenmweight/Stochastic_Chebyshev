...
GAM      = 0.05
A0_LIST  = [0.1]
WC_LIST  = np.arange( 0.5, 1.45+0.05, 0.05 )
...
for A0i,A0 in enumerate( A0_LIST ):
    for WCi,WC in enumerate( WC_LIST ):
        ...
        WC = round(WC,4)
        try:
            EGRID, DOS_PHOT[A0i,WCi,:] = np.loadtxt("PLOTS_DATA/DOS_PHOTON_N_%1.0f_Nr_%1.0f_NC_%1.0f_GAM_%1.3f_A0_%1.3f_WC_%1.3f_SIGE_%1.3f_SIGG_%1.3f_CAVLOSS_%1.3f_ABS.dat" % (N,NR,NC,GAM,A0,WC,SIGE,SIGG,CAVLOSS), dtype=np.complex64).real.T
            ...
            print("File not found for A0 = %1.3f, WC = %1.3f, CL = %1.3f" % (A0,WC,CAVLOSS))
            continue
