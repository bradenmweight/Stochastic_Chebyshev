...
def get_Globals():
    ...
    if ( SIGG > 0.0 ):
        MU[:,1,1] = np.ones( (N), dtype=np.complex64 )/100
        MU_SCALE  = np.cos(2 * np.pi * np.random.uniform( low=0.0, high=1.0, size=N ))
    ...
    GAM    = 0.01 # Broadening
    N_STOCHASTIC = 100 # Number of random vectors
...
def get_coeffs( E,E0,N_CHEB,c_l ):
    theta    = np.linspace(0,2*np.pi,800) + 0.j
    dth      = theta[1] - theta[0]
    F        = np.exp( -( dH*np.cos(theta) - E)**2 / 2 / GAM**2 )
    c_l[:]   =  F @ np.exp(1j * np.outer(theta,np.arange(N_CHEB))) * dth # Fourier Kernel
    c_l[1:]  = c_l[1:] * 2 #/ np.sqrt(2 * np.pi) #/ GAM
    return c_l
