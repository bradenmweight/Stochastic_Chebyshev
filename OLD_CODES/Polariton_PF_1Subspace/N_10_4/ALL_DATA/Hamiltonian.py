...
def get_H_vec_norm_Hermitian( N, EGS, E, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0, dH, E0, vec ):
    ...
    Hvec[0]     += np.sum( DSE_0n[:] * vec[1:N+1] ) # <g0|e0>
    Hvec[0]     += WC * A0 * np.sum( MU[:,0,0] ) * vec[N+1] # <g0|g1>
    ...
    # BOTTOM ROW
    Hvec[N+1] += WC * A0 * np.sum( MU[:,0,0] ) * vec[0]
    Hvec[N+1] += WC * A0 * np.sum( MU[:,0,1] * vec[1:N+1] )
    Hvec[N+1] += (EGS+WC-CAV_LOSS/2*1.0j+DSE_GS-E0)*vec[-1]
    
    return Hvec/dH

@jit(nopython=True)
def get_H_vec_norm_Non_Hermitian( N, EGS, E, MU, CAV_LOSS, MU2_AA, DSE_GS, DSE_DIAG, DSE_0n, WC, A0, dH, E0, vec ):
    Hvec         = np.zeros( (N+2), dtype=np.complex64 )
    Hvec[N+1] += -CAV_LOSS/2*1.0j * vec[-1]
    return Hvec/dH
